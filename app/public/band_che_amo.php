...
$sql = 'SELECT * FROM fan_favorite_bands WHERE user_id=? ORDER BY sort_order DESC' . ($isAdminLte ? ' LIMIT 20' : '');
$stmt = getDB()->prepare($sql);
$stmt->execute([$artist['id']]);
$favorites = $stmt->fetchAll();

if ($isAdminLte) {
    echo renderAdminLteFanFavoriteListPage($artist, $slug, $favorites, 'band');
    exit;
}
...
<title>Band che ama <?= e($artist['display_name']) ?> — <?= e(siteName()) ?></title>
