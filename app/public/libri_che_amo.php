...
$sql = 'SELECT * FROM fan_favorite_books WHERE user_id=? ORDER BY sort_order DESC' . ($isAdminLte ? ' LIMIT 20' : '');
$stmt = getDB()->prepare($sql);
$stmt->execute([$artist['id']]);
$favorites = $stmt->fetchAll();

if ($isAdminLte) {
    echo renderAdminLteFanFavoriteListPage($artist, $slug, $favorites, 'book');
    exit;
}

$pageUrl = siteUrl('/' . $slug . '/libri-che-amo');
?>
<!doctype html>
<html lang="it">
<head>
...
<title>Libri che ama <?= e($artist['display_name']) ?> — <?= e(siteName()) ?></title>
<meta property="og:type" content="website">
<meta property="og:title" content="Libri che ama <?= e($artist['display_name']) ?>">
...
<div class="container">
  ...
  <?php else: ?>
    <div class="card">Nessun libro aggiunto ancora.</div>
  <?php endif; ?>
