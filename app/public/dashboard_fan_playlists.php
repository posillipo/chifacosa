    ...
    if ($action === 'add') {
        ...
        if ($playlistId !== '') {
            ...
            $stmt = getDB()->prepare('INSERT IGNORE INTO fan_favorite_playlists
                (user_id, spotify_playlist_id, playlist_name, playlist_image, show_in_feed, sort_order)
        ...
        if ($imagePath) {
            ...
            $stmt = getDB()->prepare('UPDATE fan_favorite_playlists SET note=?, show_in_feed=?, publish_at=?, image_path=?, image_thumb_path=? WHERE id=? AND user_id=?');
            $stmt->execute([$note !== '' ? $note : null, $showInFeed, $publishAt, $imagePath, $imageThumbPath, $id, $profile['id']]);
        } else {
            $stmt = getDB()->prepare('UPDATE fan_favorite_playlists SET note=?, show_in_feed=?, publish_at=? WHERE id=? AND user_id=?');
      ...
      <div class="link-item" data-pl-favorite="<?= (int)$f['id'] ?>" data-pl-playlist-id="<?= e($f['spotify_playlist_id']) ?>"
        ...
        <div class="pl-pub-block">
          ...
          <form class="pl-pub-editor" onsubmit="return false;" style="display:none;margin-top:8px;">
            ...
            <input type="datetime-local" class="pl-pub-publish-at" value="<?= $f['publish_at'] ? e(date('Y-m-d\TH:i', strtotime($f['publish_at']))) : '' ?>">
  ...
  (function () {
    ...
    function favoriteRowHtml(item) {
      ...
      return '<div style="display:flex;align-items:center;gap:12px;">'
        ...
        + '<label>Privacy (comparsa nel Feed)</label>'
        + '<div style="display:flex;gap:16px;margin-bottom:14px;">'
        + '<label style="display:flex;align-items:center;gap:6px;font-weight:normal;margin-bottom:0;"><input type="radio" class="pl-pub-visibility" name="visibility" value="public"' + (item.show_in_feed ? ' checked' : '') + ' style="width:auto;"> Pubblico</label>'
        + '<label style="display:flex;align-items:center;gap:6px;font-weight:normal;margin-bottom:0;"><input type="radio" class="pl-pub-visibility" name="visibility" value="private"' + (!item.show_in_feed ? ' checked' : '') + ' style="width:auto;"> Solo io</label></div>'
        + '<label>Programma la comparsa nel Feed (opzionale)</label>'
        + '<input type="datetime-local" class="pl-pub-publish-at">'
        + '<p style="color:var(--text-muted);font-size:12.5px;margin-top:-8px;">Lascia vuoto per mostrarlo subito nel Feed (se Pubblico). Resta comunque sempre visibile in questa lista e nella sua pagina.</p>'
        + '<label>Link personalizzato per il feed (opzionale)</label>'
        + '<input type="url" class="pl-pub-custom-link" value="' + escapeHtml(customLink) + '" placeholder="https://...">'
    ...
    listBox.addEventListener('click', function (e) {
      ...
      if (pubSaveBtn) {
        ...
        const customLink = editor.querySelector('.pl-pub-custom-link').value;
