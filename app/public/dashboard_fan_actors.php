    ...
    if ($action === 'add') {
        ...
        if ($personId !== '') {
            ...
            $stmt = getDB()->prepare('INSERT IGNORE INTO fan_favorite_actors
                (user_id, tmdb_person_id, actor_name, actor_image, is_public, sort_order)
        ...
        if ($imagePath) {
            ...
            $stmt = getDB()->prepare('UPDATE fan_favorite_actors SET note=?, is_public=?, in_feed=?, publish_at=?, image_path=?, image_thumb_path=? WHERE id=? AND user_id=?');
            $stmt->execute([$note !== '' ? $note : null, $isPublic, $inFeed, $publishAt, $imagePath, $imageThumbPath, $id, $profile['id']]);
        } else {
            $stmt = getDB()->prepare('UPDATE fan_favorite_actors SET note=?, is_public=?, in_feed=?, publish_at=? WHERE id=? AND user_id=?');
    ...
    <?php foreach ($favorites as $f): ?>
      ...
      <div class="link-item" data-fa-favorite="<?= (int)$f['id'] ?>" data-fa-person-id="<?= e($f['tmdb_person_id']) ?>"
        ...
        <div class="fa-pub-badges" style="display:flex;gap:6px;flex-wrap:wrap;<?= (!$isScheduled && !$isPrivate) ? 'display:none;' : '' ?>">
          <?php if ($isScheduled): ?><span class="fa-badge-scheduled" style="background:#f0ad4e;color:#fff;font-size:11px;font-weight:700;padding:2px 8px;border-radius:999px;">⏰ Programmato per il <?= e(formatLocalDateTime($f['publish_at'], $profile)) ?></span><?php endif; ?>
        ...
        <div class="fa-pub-block">
          ...
          <form class="fa-pub-editor" onsubmit="return false;" style="display:none;margin-top:8px;">
            ...
            <label style="display:flex;align-items:center;gap:6px;font-weight:normal;">
              <input type="checkbox" class="fa-pub-in-feed" name="in_feed" value="1" <?= ($f['in_feed'] ?? 1) ? 'checked' : '' ?> style="width:auto;"> Includi nel Feed
            </label>
  ...
  (function () {
    ...
    function favoriteRowHtml(item) {
      ...
      return '<div style="display:flex;align-items:center;gap:12px;">'
        ...
        + '<label>Privacy</label>'
        + '<div style="display:flex;gap:16px;margin-bottom:14px;">'
        + '<label style="display:flex;align-items:center;gap:6px;font-weight:normal;margin-bottom:0;"><input type="radio" class="fa-pub-visibility" name="visibility" value="public"' + (item.is_public ? ' checked' : '') + ' style="width:auto;"> Pubblico</label>'
        + '<label style="display:flex;align-items:center;gap:6px;font-weight:normal;margin-bottom:0;"><input type="radio" class="fa-pub-visibility" name="visibility" value="private"' + (!item.is_public ? ' checked' : '') + ' style="width:auto;"> Solo io</label></div>'
        + '<label style="display:flex;align-items:center;gap:6px;font-weight:normal;"><input type="checkbox" class="fa-pub-in-feed" name="in_feed" value="1"' + (item.in_feed == 1 ? ' checked' : '') + ' style="width:auto;"> Includi nel Feed</label>'
        + '<p style="color:var(--text-muted);font-size:12.5px;margin:-8px 0 14px;">Non riguarda la Timeline del sito (quella segue solo Pubblico/Solo io): serve solo per le automazioni social (es. Metricool) che leggono il feed RSS del profilo.</p>'
        + '<label>Programma la pubblicazione (opzionale)</label>'
    ...
    listBox.addEventListener('click', function (e) {
      ...
      if (pubSaveBtn) {
        ...
        const inFeed = editor.querySelector('.fa-pub-in-feed').checked;
