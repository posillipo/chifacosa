...
$sql = 'SELECT * FROM fan_favorite_recipes WHERE user_id=? ORDER BY sort_order DESC' . ($isAdminLte ? ' LIMIT 20' : '');
$stmt = getDB()->prepare($sql);
$stmt->execute([$artist['id']]);
$favorites = $stmt->fetchAll();

if ($isAdminLte) {
    echo renderAdminLteFanFavoriteListPage($artist, $slug, $favorites, 'recipe');
    exit;
}

$pageUrl = siteUrl('/' . $slug . '/ricette-che-amo');
?>
<!doctype html>
<html lang="it">
<head>
...
<title>Ricette che ama <?= e($artist['display_name']) ?> — <?= e(siteName()) ?></title>
<meta property="og:type" content="website">
<meta property="og:title" content="Ricette che ama <?= e($artist['display_name']) ?>">
...
<div class="container">
  <?= publicProfileHeader($artist, 'ricettecheamo') ?>

  <div class="section-title" style="text-align:center;color:rgba(var(--text-rgb),0.6);margin:18px 0 10px;">
    Tutte le ricette che ama (<?= count($favorites) ?>)
  </div>

  <?php if ($favorites): ?>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:12px;">
      <?php foreach ($favorites as $f): ?>
        <a href="/<?= e($slug) ?>/ricette-che-amo/<?= (int)$f['id'] ?>"
           class="card" style="text-align:center;text-decoration:none;color:inherit;padding:14px 8px;">
          <?php if ($f['recipe_image']): ?>
            <img src="<?= e($f['recipe_image']) ?>" style="width:80px;height:80px;border-radius:10px;object-fit:cover;margin-bottom:8px;">
          <?php endif; ?>
          <div style="font-weight:700;font-size:13px;"><?= e($f['recipe_title']) ?></div>
          <small style="opacity:0.6;font-size:11px;"><?= e(publishedAtLabel($f['publish_at'], $f['created_at'], $artist)) ?></small>
        </a>
      <?php endforeach; ?>
    </div>
  <?php else: ?>
    <div class="card">Nessuna ricetta aggiunta ancora.</div>
  <?php endif; ?>
