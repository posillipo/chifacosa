...
$sql = 'SELECT * FROM fan_favorite_teams WHERE user_id=? ORDER BY sort_order DESC' . ($isAdminLte ? ' LIMIT 20' : '');
$stmt = getDB()->prepare($sql);
$stmt->execute([$artist['id']]);
$favorites = $stmt->fetchAll();

if ($isAdminLte) {
    echo renderAdminLteFanFavoriteListPage($artist, $slug, $favorites, 'team');
    exit;
}

$pageUrl = siteUrl('/' . $slug . '/squadre-che-amo');
?>
<!doctype html>
<html lang="it">
<head>
...
<title>Squadre che ama <?= e($artist['display_name']) ?> — <?= e(siteName()) ?></title>
<meta property="og:type" content="website">
<meta property="og:title" content="Squadre che ama <?= e($artist['display_name']) ?>">
...
<div class="container">
  <?= publicProfileHeader($artist, 'squadrecheamo') ?>

  <div class="section-title" style="text-align:center;color:rgba(var(--text-rgb),0.6);margin:18px 0 10px;">
    Tutte le squadre che ama (<?= count($favorites) ?>)
  </div>

  <?php if ($favorites): ?>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:12px;">
      <?php foreach ($favorites as $f): ?>
        <a href="/<?= e($slug) ?>/squadre-che-amo/<?= (int)$f['id'] ?>"
           class="card" style="text-align:center;text-decoration:none;color:inherit;padding:14px 8px;">
          <?php if ($f['team_badge']): ?>
            <img src="<?= e($f['team_badge']) ?>" style="width:80px;height:80px;border-radius:10px;object-fit:cover;margin-bottom:8px;">
          <?php endif; ?>
          <div style="font-weight:700;font-size:13px;"><?= e($f['team_name']) ?></div>
          <small style="opacity:0.6;font-size:11px;"><?= e(publishedAtLabel($f['publish_at'], $f['created_at'], $artist)) ?></small>
        </a>
      <?php endforeach; ?>
    </div>
  <?php else: ?>
    <div class="card">Nessuna squadra aggiunta ancora.</div>
  <?php endif; ?>
